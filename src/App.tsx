...
import {
  ...
  CircleDollarSign, Clock3, Menu, RefreshCw, ShieldCheck, WalletCards, X,
} from 'lucide-react'
import { fetchAccount } from './api'
import type { AccountView, Environment, Holding } from './types'
...
function HoldingsTable({ holdings, currency }: { holdings: Holding[]; currency: AccountView['currency'] }) {
  if (!holdings.length) return <div className="empty-state">
    ...
    <h3>보유 종목이 없습니다</h3>
    <p>선택한 모의투자 계좌에 보유 중인 종목이 없습니다.</p>
  </div>
...
function App() {
  ...
  const query = useQuery({ queryKey: ['account', environment], queryFn: () => fetchAccount(environment) })
  ...
  return <div className="app-shell">
    <header className="topbar">
      ...
      <div className="environment-picker" aria-label="투자 환경 선택">
        {environments.map((item) => <button key={item.id} disabled={item.disabled} className={environment === item.id ? 'selected' : ''} onClick={() => item.id !== 'live' && setEnvironment(item.id)}>
    ...
    <aside className={`sidebar ${mobileMenu ? 'open' : ''}`}>
      <nav><p>투자 관리</p><button className="active"><WalletCards size={19} /><span>계좌 확인</span><ChevronRight size={16} /></button></nav>
