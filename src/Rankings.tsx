...
export default function Rankings({ environment, onSelectStock }: Props) {
  ...
  return <>
    ...
    {query.data && <div className="ranking-grid">{cards.map(({ key, title, icon: Icon }) => <section className="ranking-card" key={key}><div className="ranking-card-title"><div><span><Icon size={16} /></span><h2>{title}</h2></div><small>TOP 10</small></div>{query.data[key].length ? <ol>{query.data[key].map((item) => <RankingRow key={`${item.market}-${item.code}-${item.rank}`} item={item} kind={key} overseas={overseas} onSelect={() => onSelectStock({ code: item.code, name: item.name, englishName: item.englishName, market: item.market })} />)}</ol> : <div className="ranking-empty">표시할 순위가 없습니다.</div>}</section>)}</div>}
  </>
}
